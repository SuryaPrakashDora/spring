...
public class TodoH2Service implements TodoRepository {
    ...
    public ArrayList<Todo> getTodos() {
        List<Todo> todoDate = db.query("select * from TODOLIST", new TodoRowMapper());
        ArrayList<Todo> todos = new ArrayList<>(todoDate);
        return todos;
    }
    ...
    public Todo getTodoById(int id) {
        try {
            Todo todo = db.queryForObject("select * from TODOLIST where todoId = ?", new TodoRowMapper(), id);
            return todo;
        } catch (Exception e) {
    ...
    public Todo addTodo(Todo todo) {
        ...
        Todo savedTodo = db.queryForObject("select * from TODOLIST where id = ?",
